...
#     elif isinstance(items[0], tuple) or isinstance(items[0], list):
#         idxs = [item[0] for item in items]
#         data = torch.stack([i[1] for i in items])
#         return idxs, data


# class TubeSet(Dataset):

#     def __init__(self, csvs, transforms=[], return_index=False, labels_df: pd.DataFrame = None, labels_to_return=None, sample_type_filter=None):
#         self.paths = []
#         self.labels = []
#         self.accs = []
...
#             missing_accs = 0
#             assert labels_to_return is not None and len(labels_to_return) > 0,  "Provide a label_key to query the DF"

#             if sample_type_filter is not None:
#                 labels_df = labels_df[labels_df['sample_type'] == sample_type_filter]
#                 logger.info(f"Found {len(labels_df)} samples with type {sample_type_filter}")

#             for path in csvs:
#                 acc = Path(path).name.split("_")[0]
#                 hits = labels_df.query(f"accession == '{acc}'")
#                 if len(hits) == 0:
#                     # logger.warning(f"Couldn't find accession {acc} in input DF :(")
#                     missing_accs += 1
#                     continue
#                 else:
#                     self.labels.append(hits[labels_to_return].values[0].tolist())
...
#         else:
#             self.paths = csvs
#             self.accs = [Path(self.paths[i]).name.split("_")[0] for i in range(len(self.paths))]

#         logger.info(f"Creating Dataset with {len(self.paths)} items")
...
#         #     raise ValueError(f"Inf in item {i} {self.itempath(i)} raw data")
...
#     def from_accession(self, acc):
#         idx = self.accs.index(acc)
#         return self[idx]

#     def from_accessions(self, accs):
#         return torch.stack([self.from_accession(a) for a in accs])

#     @staticmethod
#     def collate(x):
#         if type(x[0]) == tuple:
#             return [i[0] for i in x], torch.stack([i[1] for i in x], dim=0)
#         else:
