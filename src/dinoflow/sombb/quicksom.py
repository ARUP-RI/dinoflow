...
class SOM:
    ...
    def scheduler(self, it, tot):
        ...
        if self.sched == 'exp':
            return max(0.5, np.exp(-5 * it / tot))
    ...
    def fit(self,
            ...
            # custom params added by JDD
            checkpoint_freq=1,
            checkpoint_name_base="som_checkpoint",
            start_epoch=0,  # zero based
            ):
        """
        ...
        if logfile is not None:
            ...
            logfile.write('#epoch #step #iter #alpha #sigma #error #runtime\n')
        ...
        # custom change to allow start as start_epoch (zero based inside this class but logging one based)
        self.step = start_epoch * npts  # custom set step to value at start_epoch, needed for learning rate changes
        for epoch in range(start_epoch, n_epoch):  # custom range with a start epoch (zero based) if starting from a checkpoint
            epoch_err_sum = 0.0
            for i, (label, batch) in enumerate(dataloader):
                ...
                if not i % print_each:
                    ...
                    print(f'{epoch + 1}/{n_epoch}: {i}/{len(dataloader)} {self.step}/{total_steps} '
            ...
            print(f"Epoch {epoch + 1} Mean err: {epoch_err_sum / len(dataloader) :.3f}" )

            # custom save centroids every nth epoch if desired
            if checkpoint_freq and ((epoch + 1) % checkpoint_freq == 0):
                checkpoint_file = f"{checkpoint_name_base}_m{self.m}_n{self.n}_dim{self.dim}_epoch{epoch}.p"
                checkpoint_dict = dict(centroids=self.centroids, m=self.m, n=self.n)
                pickle.dump(checkpoint_dict, open(checkpoint_file, "bw"))
                #np.save(f'{checkpoint_name_base}_m{self.m}_n{self.n}_dim{self.dim}_epoch{epoch}.npy', self.centroids)
    ...
    def _get_unfold_adj(self):
        ...
        uadjmat = scipy.sparse.coo_matrix((uadjmat['data'], (uadjmat['row'], uadjmat['col'])),
                                             shape=(np.prod(ushape), np.prod(ushape)))
        self.uadj = uadjmat
