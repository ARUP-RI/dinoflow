...
class TwoHeadClassificationHead(nn.Module):
    def __init__(self, num_features, num_classes, output_scale_factor=1.0):
        super().__init__()
        self.output_scale_factor = output_scale_factor
        self.model_conf = {
    ...
    def forward(self, x):
        layer_dtype = self.layers1[0].weight.dtype
        if x.dtype != layer_dtype:
            x = x.to(layer_dtype)
        return self.layers1(x) * self.output_scale_factor, self.layers2(x)
...
def _run_trainer(model, train_labels, test_labels, tubes, run_name, labelkey, dataroot, events, batch_size, epochs, comet_workspace, comet_project, positive_repeat_factor=1, val_events=None):
...
def train(run_name, train_labels, test_labels, 
    ...
    classifier = TwoHeadClassificationHead(backbone.cls_token.shape[-1], 
    ...
    if mode == 'binary':
        model = BinaryClassificationModel(combined, emit_predictions=True, comet_project_name=comet_project, backbone_warmup_iters=20)
    ...
    if checkpoint is not None:
        ...
        if mode == 'binary':
            model = BinaryClassificationModel.load_from_checkpoint(checkpoint, backbone=backbone, classifier=classifier, backbone_warmup_iters=30)
        elif mode == 'multiclass':
    ...
    _run_trainer(model, train_labels, test_labels, [tube_type], run_name, labelkey, dataroot, events, batch_size, epochs, comet_workspace, comet_project, positive_repeat_factor, val_events)
...
def train3tubes(run_name, train_labels, test_labels,
    ...
    _run_trainer(model, train_labels, test_labels, ["b", "t", "m"], run_name, labelkey, dataroot, events, batch_size, epochs, comet_workspace, comet_project, positive_repeat_factor, val_events)
...
def predict_onetube(checkpoint: str,
        ...
        with torch.amp.autocast('cuda', enabled=True):
            ...
            for b, (batch, rowdict) in enumerate(testloader):
                ...
                for p in preds:
                    ...
                    p_str = f"{p.item():.4f}"    
                    print(f"{idx},{rowdict['ACCESSION'][i]},{p_str},{labels[i]}")
                    i += 1
                sample_start_index += len(batch)
    
    # Compute precision, recall, and F1 at F1-maximizing threshold
    all_preds = np.array(all_preds)
    all_labels = np.array(all_labels)
    
    precision_arr, recall_arr, thresholds = precision_recall_curve(all_labels, all_preds)
    # precision_recall_curve returns arrays where precision and recall have one more element than thresholds
    # The last precision/recall values are 1.0 and 0.0 respectively (at threshold > max prediction)
    # We only consider thresholds where both precision and recall are defined
    f1_scores = np.where(
    ...
    print(f"\n# F1-maximizing threshold statistics:")
