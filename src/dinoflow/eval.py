...
def train_settransformer3tube(run_name: str,
    ...
    if mode == 'binary':
        model = BinaryClassificationModel(model, emit_predictions=True, max_lr=max_lr, comet_project_name=comet_project, pos_weight=pos_weight, checkpoint_monitor=checkpoint_monitor, checkpoint_mode=checkpoint_mode)
    elif mode == 'multiclass':
        model = ClassificationModel(model, num_classes=num_classes, emit_predictions=False, max_lr=max_lr, comet_project_name=comet_project)
    elif mode == 'regression':
        model = RegressionModel(model, emit_predictions=True, max_lr=max_lr, comet_project_name=comet_project)
    else:
        raise ValueError(f"Unknown mode: {mode}")
